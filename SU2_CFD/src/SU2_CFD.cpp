 ...
 * SU2_CFD.cpp is the main control file for the SU2 tool. This is the first file a developer should look into.
 * It instantiates objects for classes for geometry (CGeometry), equation solver (CSolver), numerics class (CNumerics), output (COutput),
 * configuration file (CConfig) aomong others. Then, this file sets up and runs outer iteration loop for the equations one is interested in solving.
 * At every iteration of this outer loop, this file checks if the solution has converged and exits the loop (if it has),
 * or if we require SU2 to output a solution file at the end of this iteration, then it write the solution file, and loops on.
 *
 * Pseudocode:
 *  BEGIN
